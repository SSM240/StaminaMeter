    ...
    public class LargeStaminaMeterDisplay : BaseStaminaMeterDisplay
    {
        ...
        public override void Render()
        {
            ...
            if (drawStamina > 0f)
            {
                ...
                {
                    Draw.Rect(meterPosition, meterSize.X * (drawStamina / 110f), meterSize.Y, colorDark);
                    if (currentStamina > 0)
                    {
                        Draw.Rect(meterPosition, meterSize.X * (currentStamina / 110f), meterSize.Y, color);
                    }
                }
                else
                {
                    if (currentStamina > 0)
                    {
                        Draw.Rect(meterPosition, meterSize.X * (currentStamina / 110f), meterSize.Y, colorDark);
                    }
                    Draw.Rect(meterPosition, meterSize.X * (drawStamina / 110f), meterSize.Y, color);
                }
            }

            // low stamina marker
            Draw.Rect(meterPosition.X + meterSize.X * (20f / 110f), meterPosition.Y, 2, meterSize.Y, lineColor);
        }
    }
